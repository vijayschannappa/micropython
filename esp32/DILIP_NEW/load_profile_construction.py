...
def decode_load_survey_data(Serial_Number, Meter_Load_Data, meter_year, start, Add_time):
    try:
        Meter_Load_Data = Meter_Load_Data.split(meter_year)
        Energy = int(Meter_Load_Data[1][22:26], 16)
        R_Energy = int(Meter_Load_Data[1][28:32], 16)
        Voltage = int(Meter_Load_Data[1][34:38], 16)
        Time_Stamp = (str(zero_padding.zero_pad(str(int(meter_year, 16)),4)) + '-' +
        ...
        print('second',Error)
        no_load_survey_data_time_store(start, Add_time)

def no_load_survey_data_time_store(start, Add_time):
    print(start, Add_time)
...
def pull_load_survey_data(Serial_Number, meter_time, meter_year, Add_time, Difference_Time):  # meter_time
    try:
        for error_count in range(3):
            ...
            if tm.calculate_start_end_minus_time(start_time, 0) <= tm.calculate_start_end_minus_time(end_time, 32):
                ...
                if len(Meter_Load_Data) == string_length:
                    decode_load_survey_data(Serial_Number, Meter_Load_Data, meter_year, start_time_from_memory, Add_time)
                    break
